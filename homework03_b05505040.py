...
#function AND does the logic operation AND
def AND(u, v):
    res = ''
    if u == "0":
        res += '0'
...
#function compAND makes for larger numbers
def compAND(x, y):
    z = ''
    if len(x) != len(y):
...
#function OR does the logic operation OR
def OR(u, v):
    res = ''
    if u == "1":
        res += '1'
...
inputSTR_X = input()
inputSTR_Y = input()
outputSTR = compXOR(inputSTR_X, inputSTR_Y)
print(outputSTR)
